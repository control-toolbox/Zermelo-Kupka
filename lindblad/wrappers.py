...
def hamiltonian(delta, compile=True, display=False):
    ...
    #
    hfun   = lambda t, x, p                   : hf(x, p, delta)
    dhfun  = lambda t, x, dx, p, dp           : hf_d(x, dx, p, dp, delta)
    d2hfun = lambda t, x, dx, d2x, p, dp, d2p : hf_d_d(x, d2x, dx, p, d2p, dp, delta)
    d3hfun = lambda t, x, dx, d2x, d3x, p, dp, d2p, d3p : hf_d_d_d(x, d3x, d2x, dx, p, d3p, d2p, dp, delta)
